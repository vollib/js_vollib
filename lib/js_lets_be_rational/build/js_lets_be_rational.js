...
(function() {
  ...
  Constants = (function() {
    ...
    Constants.DBL_MIN = 2.2250738585072014e-308;

    Constants.DBL_MAX = 1.7976931348623157e+308;

    Constants.SQRT_DBL_EPSILON = Math.sqrt(Constants.DBL_EPSILON);

    Constants.FOURTH_ROOT_DBL_EPSILON = Math.sqrt(Constants.SQRT_DBL_EPSILON);

    Constants.EIGHTH_ROOT_DBL_EPSILON = Math.sqrt(Constants.FOURTH_ROOT_DBL_EPSILON);

    Constants.SQRT_DBL_EPSILON = Constants.SQRT_DBL_EPSILON;

    Constants.FOURTH_ROOT_DBL_EPSILON = Constants.FOURTH_ROOT_DBL_EPSILON;

    Constants.EIGHTH_ROOT_DBL_EPSILON = Constants.EIGHTH_ROOT_DBL_EPSILON;

    Constants.SIXTEENTH_ROOT_DBL_EPSILON = Math.sqrt(Constants.EIGHTH_ROOT_DBL_EPSILON);

    Constants.SQRT_DBL_MIN = Math.sqrt(Constants.DBL_MIN);

    Constants.SQRT_DBL_MAX = Math.sqrt(Constants.DBL_MAX);

    Constants.DENORMALIZATION_CUTOFF = 0;

    Constants.VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_BELOW_INTRINSIC = -Constants.DBL_MAX;

    Constants.VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_ABOVE_MAXIMUM = Constants.DBL_MAX;

    Constants.ONE_OVER_SQRT_TWO = 0.7071067811865475244008443621048490392848359376887;

    Constants.ONE_OVER_SQRT_TWO_PI = 0.3989422804014326779399460599343818684758586311649;

    Constants.SQRT_TWO_PI = 2.506628274631000502415765284811045253006986740610;

    Constants.TWO_PI = 6.283185307179586476925286766559005768394338798750;

    Constants.SQRT_PI_OVER_TWO = 1.253314137315500251207882642405522626503493370305;

    Constants.SQRT_THREE = 1.732050807568877293527446341505872366942805253810;

    Constants.SQRT_ONE_OVER_THREE = 0.577350269189625764509148780501957455647601751270;

    Constants.TWO_PI_OVER_SQRT_TWENTY_SEVEN = 1.209199576156145233729385505094770488189377498728;

    Constants.PI_OVER_SIX = 0.523598775598298873077107230546583814032861566563;

    return Constants;

  })();

  LetsBeRational.constants = Constants;
...
(function() {
  ...
  LetsBeRational.VolatilityValueError = VolatilityValueError;

  LetsBeRational.BelowIntrinsicError = BelowIntrinsicError;

  LetsBeRational.AboveMaximumError = AboveMaximumError;
...
(function() {
  ...
  erf_cody = (function() {
    ...
    erf_cody.erf_cody = function(x) {
      return calerf(x, 0);
    };

    erf_cody.erfc_cody = function(x) {
      return calerf(x, 1);
    };

    erf_cody.erfcx_cody = function(x) {
      return calerf(x, 2);
    };
  ...
  LetsBeRational.erf_cody = erf_cody;
...
(function() {
  ...
  normaldistribution = (function() {
    ...
    erf_cody = LetsBeRational.erf_cody;

    constants = LetsBeRational.constants;
    ...
    normaldistribution.norm_pdf = function(x) {
      return ONE_OVER_SQRT_TWO_PI * Math.exp(-0.5 * x * x);
    };

    normaldistribution.norm_cdf = function(z) {
      ...
      if (z <= norm_cdf_asymptotic_expansion_first_threshold) {
        ...
        return -normaldistribution.norm_pdf(z) * sum / z;
      }
      return 0.5 * erf_cody.erfc_cody(-z * ONE_OVER_SQRT_TWO);
    };

    normaldistribution.inverse_norm_cdf = function(u) {
      var q, r, ret;
      if (u <= 0) {
        return Math.log(u);
      }
  ...
  LetsBeRational.normaldistribution = normaldistribution;
...
(function() {
  ...
  rationalcubic = (function() {
    ...
    rationalcubic.rational_cubic_control_parameter_to_fit_second_derivative_at_left_side = function(x_l, x_r, y_l, y_r, d_l, d_r, second_derivative_l) {
    ...
    rationalcubic.minimum_rational_cubic_control_parameter = function(d_l, d_r, s, preferShapePreservationOverSmoothness) {
    ...
    rationalcubic.rational_cubic_control_parameter_to_fit_second_derivative_at_right_side = function(x_l, x_r, y_l, y_r, d_l, d_r, second_derivative_r) {
      var denominator, h, numerator;
      h = x_r - x_l;
    ...
    rationalcubic.convex_rational_cubic_control_parameter_to_fit_second_derivative_at_right_side = function(x_l, x_r, y_l, y_r, d_l, d_r, second_derivative_r, preferShapePreservationOverSmoothness) {
      var r, r_min;
      r = rationalcubic.rational_cubic_control_parameter_to_fit_second_derivative_at_right_side(x_l, x_r, y_l, y_r, d_l, d_r, second_derivative_r);
      r_min = rationalcubic.minimum_rational_cubic_control_parameter(d_l, d_r, (y_r - y_l) / (x_r - x_l), preferShapePreservationOverSmoothness);
      return Math.max(r, r_min);
    };

    rationalcubic.rational_cubic_interpolation = function(x, x_l, x_r, y_l, y_r, d_l, d_r, r) {
      var h, omt, omt2, t, t2;
      h = x_r - x_l;
      if (Math.abs(h) <= 0) {
        return 0.5 * (y_l + y_r);
    ...
    rationalcubic.convex_rational_cubic_control_parameter_to_fit_second_derivative_at_left_side = function(x_l, x_r, y_l, y_r, d_l, d_r, second_derivative_l, preferShapePreservationOverSmoothness) {
      var r, r_min;
      r = rationalcubic.rational_cubic_control_parameter_to_fit_second_derivative_at_left_side(x_l, x_r, y_l, y_r, d_l, d_r, second_derivative_l);
      r_min = rationalcubic.minimum_rational_cubic_control_parameter(d_l, d_r, (y_r - y_l) / (x_r - x_l), preferShapePreservationOverSmoothness);
      return Math.max(r, r_min);
    };
  ...
  LetsBeRational.rationalcubic = rationalcubic;
...
(function() {
  ...
  lets_be_rational = (function() {
    var AboveMaximumError, BelowIntrinsicError, DBL_EPSILON, DBL_MAX, DBL_MIN, DENORMALIZATION_CUTOFF, EIGHTH_ROOT_DBL_EPSILON, FOURTH_ROOT_DBL_EPSILON, ONE_OVER_SQRT_TWO, ONE_OVER_SQRT_TWO_PI, PI_OVER_SIX, SIXTEENTH_ROOT_DBL_EPSILON, SQRT_DBL_EPSILON, SQRT_DBL_MAX, SQRT_DBL_MIN, SQRT_ONE_OVER_THREE, SQRT_PI_OVER_TWO, SQRT_THREE, SQRT_TWO_PI, TWO_PI, TWO_PI_OVER_SQRT_TWENTY_SEVEN, VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_ABOVE_MAXIMUM, VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_BELOW_INTRINSIC, _asymptotic_expansion_of_normalized_black_call, _compute_f_lower_map_and_first_two_derivatives, _compute_f_upper_map_and_first_two_derivatives, _householder_factor, _inverse_f_lower_map, _inverse_f_upper_map, _is_below_horizon, _normalised_black_call_using_erfcx, _normalised_intrinsic, _normalised_intrinsic_call, _normalized_black_call_using_norm_cdf, _small_t_expansion_of_normalized_black_call, _square, _unchecked_normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations, asymptotic_expansion_accuracy_threshold, constants, erf_cody, implied_volatility_maximum_iterations, normaldistribution, rationalcubic, small_t_expansion_of_normalized_black_threshold;

    function lets_be_rational() {}

    normaldistribution = LetsBeRational.normaldistribution;

    erf_cody = LetsBeRational.erf_cody;

    rationalcubic = LetsBeRational.rationalcubic;

    constants = LetsBeRational.constants;

    BelowIntrinsicError = LetsBeRational.BelowIntrinsicError;

    AboveMaximumError = LetsBeRational.AboveMaximumError;
    ...
    _unchecked_normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations = function(beta, x, q, N) {
      ...
      b_c = lets_be_rational.normalised_black_call(x, s_c);
      v_c = lets_be_rational.normalised_vega(x, s_c);
      if (beta < b_c) {
        s_l = s_c - b_c / v_c;
        b_l = lets_be_rational.normalised_black_call(x, s_l);
        if (beta < b_l) {
          ...
          while (iterations < N && Math.abs(ds) > DBL_EPSILON * s) {
            ...
            ds_previous = ds;
            b = lets_be_rational.normalised_black_call(x, s);
            bp = lets_be_rational.normalised_vega(x, s);
            if (b > beta && s < s_right) {
              s_right = s;
          ...
          v_l = lets_be_rational.normalised_vega(x, s_l);
        ...
        b_h = lets_be_rational.normalised_black_call(x, s_h);
        if (beta <= b_h) {
          v_h = lets_be_rational.normalised_vega(x, s_h);
    ...
    lets_be_rational.normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations = function(beta, x, q, N) {
    ...
    lets_be_rational.implied_volatility_from_a_transformed_rational_guess_with_limited_iterations = function(price, F, K, T, q, N) {
    ...
    lets_be_rational.normalised_implied_volatility_from_a_transformed_rational_guess = function(beta, x, q) {
      ...
      :rtype: float
       */
      return lets_be_rational.normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(beta, x, q, implied_volatility_maximum_iterations);
    };

    lets_be_rational.implied_volatility_from_a_transformed_rational_guess = function(price, F, K, T, q) {
      ...
      :rtype: float
       */
      return lets_be_rational.implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(price, F, K, T, q, implied_volatility_maximum_iterations);
    };

    lets_be_rational.normalised_vega = function(x, s) {
    ...
    lets_be_rational.normalised_black_call = function(x, s) {
      ...
      var ax;
      if (x > 0) {
        return _normalised_intrinsic_call(x) + lets_be_rational.normalised_black_call(-x, s);
      }
      ax = Math.abs(x);
    ...
    lets_be_rational.normalised_black = function(x, s, q) {
      ...
      :rtype: float
       */
      return lets_be_rational.normalised_black_call((q < 0 ? -x : x), s);
    };

    lets_be_rational.black = function(F, K, sigma, T, q) {
      ...
      intrinsic = Math.abs(Math.max((q < 0 ? K - F : F - K), 0.0));
      if (q * (F - K) > 0) {
        return intrinsic + lets_be_rational.black(F, K, sigma, T, -q);
      }
      return Math.max(intrinsic, (Math.sqrt(F) * Math.sqrt(K)) * lets_be_rational.normalised_black(Math.log(F / K), sigma * Math.sqrt(T), q));
    };
  ...
  LetsBeRational.lets_be_rational = lets_be_rational;
...
(function() {
  ...
  js_lets_be_rational = (function() {
    var lets_be_rational, normaldistribution;

    function js_lets_be_rational() {}

    lets_be_rational = LetsBeRational.lets_be_rational;

    normaldistribution = LetsBeRational.normaldistribution;

    js_lets_be_rational.black = function(F, K, sigma, T, q) {
      return lets_be_rational.black(F, K, sigma, T, q);
    };

    js_lets_be_rational.implied_volatility_from_a_transformed_rational_guess = function(price, F, K, T, q) {
      return lets_be_rational.implied_volatility_from_a_transformed_rational_guess(price, F, K, T, q);
    };

    js_lets_be_rational.implied_volatility_from_a_transformed_rational_guess_with_limited_iterations = function(price, F, K, T, q, N) {
      return lets_be_rational.implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(price, F, K, T, q, N);
    };

    js_lets_be_rational.normalised_black = function(x, s, q) {
      return lets_be_rational.normalised_black(x, s, q);
    };

    js_lets_be_rational.normalised_black_call = function(x, s) {
      return lets_be_rational.normalised_black_call(x, s);
    };

    js_lets_be_rational.normalised_implied_volatility_from_a_transformed_rational_guess = function(beta, x, q) {
      return lets_be_rational.normalised_implied_volatility_from_a_transformed_rational_guess(beta, x, q);
    };

    js_lets_be_rational.normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations = function(beta, x, q, N) {
    ...
    js_lets_be_rational.normalised_vega = function(x, s) {
      return lets_be_rational.normalised_vega(x, s);
    };

    js_lets_be_rational.norm_cdf = function(z) {
  ...
  window.js_lets_be_rational = js_lets_be_rational;

  js_lets_be_rational.VolatilityValueError = LetsBeRational.VolatilityValueError;

  js_lets_be_rational.BelowIntrinsicError = LetsBeRational.BelowIntrinsicError;

  js_lets_be_rational.AboveMaximumError = LetsBeRational.AboveMaximumError;
